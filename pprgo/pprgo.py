...
class SAPPRGo(nn.Module):
    def __init__(self, num_features, num_classes, num_ppr, hidden_size, nlayers, dropout):
        super().__init__()
        self.num_ppr = num_ppr
        self.mlp = PPRGoMLP(num_features, num_classes, hidden_size, nlayers, dropout)
        self.linear_before_squeeze = nn.Linear(num_ppr, 1)

    def forward(self, X_list, ppr_scores_list, ppr_idx_list):
        ...
        x_3d = torch.stack(propagated_logits_list, dim=2)
        x_2d = self.linear_before_squeeze(x_3d).squeeze()
        return x_2d
